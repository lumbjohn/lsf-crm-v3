...
class CrmAjaxController extends Controller
{
    public function handleAction(Request $request)
    {
        ...
        if (!isset($request_input['action'])) {
            return ['code' => 'ERROR', 'message' => 'missing action'];
        }
        ...
        switch ($action) {
            ...
            case 'check-recall':
//                if (CrmUser::isAdmin($currentUser)) {
//                    return errorJSON(['message' => 'utilisateur admin']);
//                }

                if (!checkFields($request_input, ['tm'])) {
                    return errorJSON(['message' => 'Informations incorrectes']);
                }

                $dt = dtformat(explode('(', $request_input['tm'])[0]);
                $recall = Comment::getFirstRecall($dt, $currentUser->id_crmuser);

                if ($recall) {
                    return successJSON(['data' => $recall]);
                }

                return errorJSON(['message' => 'pas de rappel']);
                break;
        ...
        return ['code' => 'ERROR', 'message' => 'no return', 'details' => ['action' => $action]];
    }
}
