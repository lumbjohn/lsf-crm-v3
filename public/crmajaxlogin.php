...
function handleLogin($request_input)
{
    if (!checkFields($request_input, array('email', 'psw'))) {
        echo json_encode(errorJSON(array('message' => 'Champs obligatoires manquants')));
    }
    ...
    if (!$user) {
        ...
        echo json_encode(errorJSON(array('message' => 'Utilisateur non enregistré')));
    }
    ...
    if ($user->psw === md5($request_input['psw'])) {
        ...
        echo json_encode(successJSON(array('OK' => 'OK')));
    } else {
        ...
        echo json_encode(errorJSON(array('message' => 'Informations incorrectes')));
    }
}
